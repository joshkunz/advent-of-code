...
fn n_bags(which: &Bag, m: &HashMap<&Bag, &Rule>) -> i64 {
    // The bag itself, so we start at 1.
    let mut count: i64 = 1;
    for c in &m[which].clauses {
        count += c.count * n_bags(&c.bag, m);
    }
    return count;
}
...
fn main() {
    ...
    let solution = n_bags(&Bag::from("shiny gold"), &rmap);

    // -1 because we don't count the shiny gold bag itself.
    println!("Count: {}", solution - 1);
}
