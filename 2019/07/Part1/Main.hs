...
runAmplifier :: [(Int, Computer)] -> Either String Computer
runAmplifier [] = Left $ "No computers given to simulate"
runAmplifier cs = 
    rec 0 cs
    where rec lastOut [(input,c)] = runC [input, lastOut] c
          rec lastOut ((input,c):rest) =
            runC [input, lastOut] c >>= \cout -> rec (head $ output cout) rest

thrust :: Computer -> [Int] -> Either String Int
thrust comp is = fmap (head . output) $ runAmplifier $ zip is (repeat comp) 

solve :: Computer -> Either String (Int, [Int])
solve computer =
    fmap (maximumBy byThrust) $ sequence $ calcThrust <$> permutations [0..4]
    where calcThrust i = (\t -> (t, i)) <$> thrust computer i
          byThrust (t1,_) (t2,_) = compare t1 t2

fromEither :: Either a a -> a
fromEither (Left v) = v
fromEither (Right v) = v

main = interact $ either id show . solve . parse
